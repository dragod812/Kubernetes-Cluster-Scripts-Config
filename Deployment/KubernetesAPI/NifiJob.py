...
def getBody(namespace='couture-console', jobname='nifi-test', containername='nifi-test', containerimage='sidharthc/nifi-test:alpha', env_vars=ENV_LIST, containerargs=['SFTP_TO_HDFS.py']):
...
try:
    api_response = api_instance.create_namespaced_job("couture-console", getBody(), pretty=True)
    print(api_response)
